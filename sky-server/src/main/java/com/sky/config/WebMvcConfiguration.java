...
public class WebMvcConfiguration extends WebMvcConfigurationSupport {
    ...
    protected void addInterceptors(InterceptorRegistry registry) {
        log.info("开始注册自定义拦截器...");
        registry.addInterceptor(jwtTokenAdminInterceptor)//拦截管理端的拦截器
                .addPathPatterns("/admin/**")
                .excludePathPatterns("/admin/employee/login");

        registry.addInterceptor(jwtTokenUserInterceptor)//拦截客户端的拦截器
     ...
     * @param registry
     */
    protected void addResourceHandlers(ResourceHandlerRegistry registry) {//规定文档生成后保存的位置，注意给方法名是重写方法，名字固定
        log.info("开始设置静态映射");
    ...
    protected void extendMessageConverters(List<HttpMessageConverter<?>> converters) {
        ...
        //将自己的消息转换器加入到容器中
        converters.add(0,converter);//0表示将自己的转换器放在最优先
    }
}
